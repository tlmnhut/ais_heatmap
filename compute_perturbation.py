...
from utils import pearsonr_2d, cosine_sim_2d, spearman_2d, get_behavior_data, get_fmri_data
...
def corr_modified_vs_original(modified_mat, original_mat):
    ...
    for i in tqdm(range(original_mat.shape[0])):  # 120
        ...
        for j in range(modified_mat.shape[1]):  # 512
            ...
            corr_scores = cosine_sim_2d(vec=modified_mat[i, j], mat=original_mat)
...
def compute_perturbation(second_oi_original, first_oi_modified, corr_supervised):
    ...
    for i in range(corr_supervised.shape[0]):  # 120 images
        second_oi_modified = spearman_2d(np.delete(corr_supervised[i], i), np.delete(first_oi_modified[i], i, axis=1))
...
if __name__ == '__main__':
    ...
    for dataset in dataset_names:
        ...
        pathlib.Path(f'./res/corr/{trained_dataset}/remove_fm/').mkdir(parents=True, exist_ok=True)
        np.save(f'./res/corr/{trained_dataset}/remove_fm/{dataset}', corr_modified)

        # original_rsm = np.corrcoef(original_penultimate)
        original_rsm = cosine_similarity(original_penultimate)
        corr_baseline_individual = []
        for i in range(original_rsm.shape[0]):
            corr_baseline_individual.append(spearmanr(np.delete(original_rsm[i], i), np.delete(corr_human[i], i))[0])
            # corr_baseline_individual.append(pearsonr(original_rsm[i], corr_human[i])[0])
        pathlib.Path(f'./res/corr/{trained_dataset}/original/').mkdir(parents=True, exist_ok=True)
        np.save(f'./res/corr/{trained_dataset}/original/{dataset}', np.array(corr_baseline_individual))

        corr_baseline_individual = np.load(f'./res/corr/{trained_dataset}/original/{dataset}.npy')
        corr_remove_individual = np.load(f'./res/corr/{trained_dataset}/remove_fm/{dataset}.npy')
        ...
        pathlib.Path(f'./res/scores/{trained_dataset}/remove_fm/').mkdir(parents=True, exist_ok=True)
        np.save(f'./res/scores/{trained_dataset}/remove_fm/{dataset}_pert_scores', perturbation_scores)
